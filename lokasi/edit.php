...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if ($status != $data['status_lokasi']) {
        $cek = $conn->prepare("
            SELECT COUNT(*) AS jumlah
            FROM penyewaan
            WHERE id_lokasi = ?
            AND status_penyewaan IN ('Pending','Aktif')
        ");
        ...
        if ($jumlah > 0) {
            echo "<script>
                    alert('Masih ada penyewaan aktif pada lokasi ini');
                    window.history.back();
                  </script>";
            exit;
        }
    ...
    if (!empty($_FILES['foto_lokasi']['name'])) {
        ...
        // Hapus foto lama
        if (!empty($data['foto_lokasi'])) {
            $file_lama = "../uploads/lokasi/" . $data['foto_lokasi'];
            if (file_exists($file_lama)) {
                unlink($file_lama);
            }
        }

        $nama_foto = time() . '_' . $_FILES['foto_lokasi']['name'];
        move_uploaded_file($_FILES['foto_lokasi']['tmp_name'], "../uploads/lokasi/" . $nama_foto);
    }

    $update = $conn->prepare("UPDATE lokasi SET nama_lokasi = ?, alamat = ?, id_jenis = ?, ukuran = ?, harga_per_hari = ?, status_lokasi = ?, foto_lokasi = ? WHERE id = ?");
    $update->bind_param("ssisissi", $nama_lokasi, $alamat, $id_jenis, $ukuran, $harga, $status, $nama_foto, $id);
    ...
    <div class="p-6">
        <form id="formEditLokasi" action="" method="POST" enctype="multipart/form-data" class="space-y-5">
            <div>
                ...
                <input type="text" value="<?= $data_lokasi['kode_lokasi'] ?>" disabled
            ...
            <div>
                ...
                <input type="text" name="nama_lokasi" required value="<?= $data_lokasi['nama_lokasi'] ?>"
            ...
            <div>
                ...
                <textarea name="alamat" required rows="3"
                    class="w-full px-4 py-2 border border-gray-300 rounded-lg"><?= $data_lokasi['alamat'] ?></textarea>
            </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                <div>
                    ...
                    <select name="id_jenis" required
                        ...
                        <?php while ($jenis = $ambil_jenis->fetch_assoc()): ?>
                            <option value="<?= $jenis['id'] ?>" <?php if ($data_lokasi['id_jenis'] == $jenis['id'])
                                  echo 'selected'; ?>>
                                <?= $jenis['nama_jenis'] ?>
                            </option>
                        <?php endwhile; ?>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                <div>
                    ...
                    <input type="number" name="harga_per_hari" required min="0"
                        value="<?= $data_lokasi['harga_per_hari'] ?>"
                        class="w-full px-4 py-2 border border-gray-300 rounded-lg">
                </div>
                <div>
                    ...
                    <select name="status_lokasi" required
                        class="w-full px-4 py-2 border border-gray-300 rounded-lg bg-white">
                        <option value="tersedia" <?php if ($data_lokasi['status_lokasi'] == 'tersedia')
                            echo 'selected'; ?>>Tersedia</option>
                        <option value="digunakan" <?php if ($data_lokasi['status_lokasi'] == 'digunakan')
                            echo 'selected'; ?>>Digunakan</option>
                        <option value="maintenance" <?php if ($data_lokasi['status_lokasi'] == 'maintenance')
            ...
            <div>
                ...
                <?php if ($data_lokasi['foto_lokasi']): ?>
                    <div class="mb-2">
                        <img src="<?= BASE_URL ?>/uploads/lokasi/<?= $data_lokasi['foto_lokasi'] ?>" alt="Foto"
